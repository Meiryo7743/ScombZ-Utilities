const scombz = () => {
  ...
  if (document.domain === 'scombz.shibaura-it.ac.jp') {
    // テスト改善
    if (
      location.href.indexOf('examination') !== -1 &&
        ...
        if ($exa_img) {
          ...
          if ($exa_timer) {
            $exa_timer.style.width =
              document.body.clientWidth - $exa_examImgList[0].clientWidth + 'px'
          }
        }
    ...
    if ($closeButton && s2b($settings_exit_auto)) {
      ...
      if (
        !document
          ...
          .classList.contains('sidemenu-close')
      ) {
    ...
    window.addEventListener('load', function () {
      ...
      if ($settings_submenu) {
        ...
        if (s2b($settings_additional_lms)) {
          if (
            ...
            if ($courseList[0]) {
              ...
              localStorage.setItem(
                'udai:timetableDataList',
                encodeURIComponent($timetableData)
              )
          ...
          if (localStorage.getItem('udai:timetableDataList') == null) {
            ...
            const $timetableDataStr = decodeURIComponent(
              localStorage.getItem('udai:timetableDataList')
            )
            console.log(
              'LocalStrageのアクセスに成功しました\nJSONファイルにparseします'
            )
        ...
        // お知らせを変更する
        const $sidemenuInfoList = document.querySelectorAll(
          '.sidemenu-link.info-icon'
        )
        ...
        // シラバスリンク
        if (
          location.href.indexOf('scombz.shibaura-it.ac.jp/lms/course?idnumber=')
        ) {
          console.log(
            '授業別ページを検出しました\nシラバスのデータと連携します'
          )
          ...
          if ($courseTitle) {
            ...
            if ($courseName.search(/[０-９]|[0-9]/) > 0) {
              $courseNameStr = $courseName.slice(
                0,
                $courseName.search(/[０-９]|[0-9]/)
              )
              $courseNameStr =
                $courseNameStr +
                ' ' +
            ...
            if (
              ...
              $courseTitle.parentNode.insertAdjacentHTML(
                'beforeEnd',
                `<span style="color:red;">シラバス表示をするには、学年と学部を設定してください</span>`
              )
            } else {
              console.log('UnicodeからEUC-JPエスケープに変換中です')
              // EUC-JP に変換
              actual = Encoding.convert(
                $courseNameStr.split('').map((v) => v.charCodeAt()),
                'EUCJP',
                'UNICODE'
              )
              // URL用にエスケープ
              ...
              $courseTitle.parentNode.insertAdjacentHTML(
                'beforeEnd',
                `<a href="http://syllabus.sic.shibaura-it.ac.jp/namazu/namazu.cgi?ajaxmode=true&query=%2B` +
                  escaped +
                  `&whence=0&idxname=` +
                  $settings_year +
                  `%2F` +
                  $settings_fac +
                  `&max=20&result=normal&sort=score#:~:text=%E6%A4%9C%E7%B4%A2%E7%B5%90%E6%9E%9C,-%E5%8F%82%E8%80%83%E3%83%92%E3%83%83%E3%83%88%E6%95%B0"  target="_blank" rel="noopener noreferrer" class="btn btn-square btn-square-area btn-txt white-btn-color" style="margin-left:40px;margin-bottom:5px;">シラバスを表示</a>
